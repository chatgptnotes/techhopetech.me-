-- Proposal approval queue and file storage.
create table if not exists public.bni_proposals (
  id             uuid primary key default gen_random_uuid(),
  contact_id     text not null references public.bni_contacts(id) on delete cascade,
  assignee_id    uuid,
...
alter table public.bni_proposals enable row level security;
drop policy if exists "bni_proposals_anon_all" on public.bni_proposals;
create policy "bni_proposals_anon_all" on public.bni_proposals
